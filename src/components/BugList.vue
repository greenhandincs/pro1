    ...
    <div id="bug-list">
        <table>
            <tr>
                ...
                <th>全选<input type="checkbox" v-model="isCheckAll"></th>
                <th>Bug描述</th>
                <th>操作</th>
            </tr>
            <BugItem v-for="bug in bugs" :key="bug.id" :bug="bug" :solveBugCallBack="solveBugCallBack"
                :delBugCallBack="delBugCallBack" ></BugItem>
...
export default {
    ...
    computed: {
        isCheckAll: {
            get() {                
                return this.bugs.filter(bug => bug.checked).length == this.bugs.length
            },
            set(val) {
                this.checkAllCallBack(val)
            }
        }
    },
    // updated() {
